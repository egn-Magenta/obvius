...
function TinyMCE_table_getControlHTML(control_name) {
	var controls = new Array(
		...
		['cell_before', 'table_insert_cell_before.gif', '{$table_insert_cell_before_desc}', 'mceTableInsertCellBefore'],
		['cell_after', 'table_insert_cell_after.gif', '{$table_insert_cell_after_desc}', 'mceTableInsertCellAfter'],
	...
	if (control_name == "tablecontrols") {
		...
		//Chris Benjaminsen BeIT ApS
		html += tinyMCE.getControlHTML("separator");
		html += tinyMCE.getControlHTML("convert_td_to_th");
		html += tinyMCE.getControlHTML("convert_th_to_td");


		html += tinyMCE.getControlHTML("separator");
		html += tinyMCE.getControlHTML("cell_before");
		html += tinyMCE.getControlHTML("cell_after");
		html += tinyMCE.getControlHTML("delete_cell");
...
//	tinyMCE.selectedInstance.execCommand("mceAddUndoLevel");
	switch (command) {
		...
		case "mceInsertTable":
			if (user_interface) {
				...
				if (tinyMCE.tableElement) {
					...
					var captionElement = tinyMCE.tableElement.getElementsByTagName('caption')[0]
				...
				if (tinyMCE.tableElement) {
					...
					//Chris Benjaminsen BeIT ApS **
					tinyMCE.tableElement.setAttribute('summary', summary);
					
					if( caption ){
						var captionElement = tinyMCE.tableElement.createCaption()
						captionElement.innerHTML = caption
						//Render fix for mozilla
						if( tinyMCE.isGecko ) 
		...
		case "mceTableConvertTdToTh":
		case "mceTableConvertThToTd":
		case "mceTableInsertCellBefore":
		case "mceTableInsertCellAfter":
		case "mceTableDeleteCell":
				...
				switch (command) {
					case "mceTableConvertTdToTh":
						if( tdElement ){
							...
							tdElement.parentNode.insertBefore(newTH,tdElement.nextSibling)
					...
					case "mceTableConvertThToTd":
						if( thElement ){
							...
							thElement.parentNode.insertBefore(newTD,thElement.nextSibling)
					...
					case "mceTableInsertCellAfter":
						...
						if (tableBorder == 0)
							newTD.style.cssText = visualAidStyle;
						if( tdElement ) 
							tdElement.parentNode.insertBefore(newTD,tdElement.nextSibling)
					...
					case "mceTableInsertCellBefore":
						...
						if (tableBorder == 0)
							newTD.style.cssText = visualAidStyle;
						if( tdElement ) 
							tdElement.parentNode.insertBefore(newTD,tdElement)
					...
					case "mceTableMergeCells":
						...
						if (user_interface) {
							// Setup template
							var template = new Array();
							template['file'] = '../../plugins/table/mergeCells.htm';
							template['width'] = 340;
							template['height'] = 220;
							...
							// Open window
							tinyMCE.openWindow(template, {editor_id : inst.editorId, maxWidth : rowCount, maxHeight: collCount});
						} else {
							var rows = []
							var cells = []
							...
							for( var row=0;row<value['height'];row++){
								...
								for( var a=0;a<toHandle.length;a++){
									var cN = toHandle[a];
									curIndex += cN.colSpan || 1;
									if( curIndex < totalScanWidth && curIndex >= startAt ){
										cells.push(cN)	
									}
								}
								var h = 100
							...
							for( var a=1;a<cells.length;a++){
								innerHTML += cells[a].innerHTML
								cells[a].parentNode.removeChild(cells[a])
							}
							cells[0].colSpan = value['width']
							cells[0].rowSpan = value['height']
							cells[0].innerHTML += innerHTML
...
function TinyMCE_table_handleNodeChange(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
	...
	//Chris Benjaminsen BeIT ApS **
	tinyMCE.switchClassSticky(editor_id + '_cell_before', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(editor_id + '_cell_after', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(editor_id + '_delete_cell', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(editor_id + '_merge_cells', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(editor_id + '_convert_td_to_th', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(editor_id + '_convert_th_to_td', 'mceButtonDisabled', true);
	//**
	...
	if (tinyMCE.getParentElement(node, "td")) {
		...
		//Chris Benjaminsen BeIT ApS **
		tinyMCE.switchClassSticky(editor_id + '_cell_before', 'mceButtonNormal', false);
		tinyMCE.switchClassSticky(editor_id + '_cell_after', 'mceButtonNormal', false);
		tinyMCE.switchClassSticky(editor_id + '_delete_cell', 'mceButtonNormal', false);
		tinyMCE.switchClassSticky(editor_id + '_merge_cells', 'mceButtonNormal', false);
		tinyMCE.switchClassSticky(editor_id + '_convert_td_to_th', 'mceButtonNormal', false);
		//**
	}
