...
function onLoadInit() {
    ...
    // IE7 is broken, so disable all the fancy stuff
    if(tinymce.isIE && (tinymce.isIE7 || document.documentMode == 7)) {
	document.getElementById('highlighting').checked = "";
	the.coloring_active = false;
	document.getElementById("wrapline").style.display = 'none';
	document.getElementById("undo").parentNode.style.display = 'none';
    }
...
function saveContent() {
    var content = '';
    if(document.getElementById('highlighting').checked) {
	content = pluginCodeMirror.getValue();
    } else {
	content = document.getElementById('htmlSource').value;
    }
        // Add an undo level

    tinyMCEPopup.editor.execCommand("mceSetContent", null, content);
    //tinyMCEPopup.editor.setContent(content, { source_view : true });
    //console.log(tinyMCEPopup.editor.undoManager.add())
    tinyMCEPopup.close();
}
...
function resizeInputs(id) {
	...
	var divs = document.getElementsByTagName('div'), scrollDiv;
	for(var i=0;i<divs.length;i++) {
	    if((divs[i].className || '').match(/CodeMirror-scroll/)) {
		scrollDiv = divs[i];
		break;
	    }
	}
	if(scrollDiv) {
	    scrollDiv.style.width = (vp.w - 20) + "px";
	    scrollDiv.style.height = (vp.h - 75) + "px";
	}
}
...
function beautify(id) {
    ...
    document.getElementById(id).value = style_html(
	source,
	{
	    'indent_size': indent_size,
	    'indent_char': indent_char,
	    'max_char': 100,
	    'brache_style': brace_style,
	    'unformatted': [
		'a',
		'b',
		'strong',
