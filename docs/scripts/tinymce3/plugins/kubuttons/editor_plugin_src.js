(function() {
	...
	init : function(ed, url) {
		...
		var cb = ed.settings['dialog_open_callback'];
		if(cb)
		    ed.windowManager.onOpen.add(cb);
	    });
	    ...
	    */
	    ...
	    // Event handler for deleting images with links on them.
		...
		if (
		    (e.keyCode == DELETE || e.keyCode == BACKSPACE) &&
		    !VK.modifierPressed(e)) {
		    var n = ed.selection.getNode();
		    if(n && n.nodeName == 'IMG' &&
		       n.parentNode && n.parentNode.nodeName == 'A') {
			e.preventDefault();
			ed.dom.remove(n.parentNode, false);
		    }
		};
	    });
	},
