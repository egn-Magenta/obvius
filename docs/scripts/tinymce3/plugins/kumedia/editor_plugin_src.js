...
(function() {
	...
	tinymce.create('tinymce.plugins.KuMediaPlugin', {
		init : function(ed, url) {
			...
			ed.addCommand('mceMedia', function() {
				...
				ed.windowManager.open({
					file : url + '/media.htm',
					width : 620 + parseInt(ed.getLang('media.delta_width', 0)),
					height : 550 + parseInt(ed.getLang('media.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url,
					data : data
			...
			// Register buttons
			ed.addButton('kumedia', {title : 'media.desc', cmd : 'mceMedia', 'class': 'mce_media'});

			// Update media selection status
			ed.onNodeChange.add(function(ed, cm, node) {
				cm.setActive('kumedia', isMediaImg(node));
			});
		},
		...
		dataToImg : function(data, force_absolute) {
			...
			img.width = data.width || (data.type == 'audio' ? "300" : "496");
			img.height = data.height || (data.type == 'audio' ? "32" : "284");
		...
		objectToImg : function(node) {
			...
			img.attr({
				...
				style : style,
				width : width || (node.name == 'audio' ? "300" : "505"),
				height : height || (node.name == 'audio' ? "32" : "284"),
				hspace : hspace,
				vspace : vspace,
				align : align,
				bgcolor : bgcolor,
	...
	// Register plugin
	tinymce.PluginManager.add('kumedia', tinymce.plugins.KuMediaPlugin);
})();
