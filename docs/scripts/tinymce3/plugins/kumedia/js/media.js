(function() {
	...
	function getVal(id) {
		...
		// Hack to remove extra slash on Obvius URLs
		if(id == 'src') {
			elm.value = elm.value.replace(/(\.[^\.]+)\/$/, '$1');
		}
	...
	window.Media = {
		...
		insert : function() {
			...
			editor.selection.setNode(editor.plugins.kumedia.dataToImg(this.data));
			tinyMCEPopup.close();
		},

		preview : function() {
			get('prev').innerHTML = this.editor.plugins.kumedia.dataToHtml(this.data, true);
		},

		moveStates : function(to_form, field) {
			var data = this.data, editor = this.editor,
				mediaPlugin = editor.plugins.kumedia, ext, src, typeInfo, defaultStates, src;
			...
			if (to_form) {
				...
				// Set default size
                setVal('width', data.width || (data.type == 'audio' ? 300 : 505));
                setVal('height', data.height || (data.type == 'audio' ? 32 : 284));
			}
		},
		...
		formToData : function(field) {
			...
			if (field == 'source') {
				...
				setVal('source', this.editor.plugins.kumedia.dataToHtml(this.data));
				this.panel = 'source';
			} else {
				if (this.panel == 'source') {
					this.data = clone(this.editor.plugins.kumedia.htmlToData(getVal('source')));
					this.dataToForm();
					this.panel = '';
				}
		...
		beforeResize : function() {
            this.width = parseInt(getVal('width') || (this.data.type == 'audio' ? "300" : "505"), 10);
            this.height = parseInt(getVal('height') || (this.data.type == 'audio' ? "32" : "284"), 10);
		},
		...
		changeSize : function(type) {
			...
			if (get('constrain').checked) {
                width = parseInt(getVal('width') || (this.data.type == 'audio' ? "300" : "505"), 10);
                height = parseInt(getVal('height') || (this.data.type == 'audio' ? "32" : "284"), 10);
		...
		getMediaTypeHTML : function(editor) {
			var type_to_extensions = new Object();
			tinymce.each(editor.getParam("media_types",
				...
				item = item.split(/=/);
				type_to_extensions[item[0].toLowerCase()] = item[1].toLowerCase();
			});
			function option(media_type){
				var ext = type_to_extensions[media_type] || '';
				if(ext)
					ext = " (" + ext + ")"
				return '<option value="'+media_type+'">'+tinyMCEPopup.editor.translate("media_dlg."+media_type)+ext+'</option>'
			}
			var html = "";
