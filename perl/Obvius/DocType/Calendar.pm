...
sub action {
    ...
    my $nothidden;
    if(defined($obvius->config->param('show_hidden_calendarevents'))) {
        $nothidden = ! $obvius->config->param('show_hidden_calendarevents');
    } else {
        $nothidden = !$is_admin;
    }
    ...
    if($show_as eq 'list') {
        ...
        for my $e (@$events) {
            ...
            next unless(check_date($start_year, $start_month, $start_day));
            ...
            if($endtime and $endtime ne $time and $endtime =~ /^(\d\d\d\d)-(\d\d)-(\d\d)/) {
                ...
                if(check_date($end_year, $end_month, $end_day)) {
