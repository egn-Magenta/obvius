...
sub update_doctypes {
    ...
    eval {
        ...
        my ($doctypemap, $fieldspecmap) = add_doctypes($doctypes_txt, $dbh, $fieldtypemap);
...
sub add_fieldtypes {
    ...
    my $updater = $dbh->prepare('update fieldtypes set ' . join(", ", map { "${_} = ?" } @fields) . ' where id = ?');

    my $inserter =
      $dbh->prepare(
        'insert into fieldtypes (' . join(", ", @fields) . ') values (' . join(", ", map { "?" } @fields) . ')');
    ...
    while (<FH>) {
        ...
        if (my $existing = $fieldtypemap{$fieldtype{name}}) {
            ...
            my %data    = map { $_ => ($fieldtype{$_} || $defaults{$_}) } @fields;
            ...
            $inserter->execute(map { $fieldtype{$_} || $defaults{$_} } @fields,);
...
sub add_doctypes {
    ...
    my $inserter =
      $dbh->prepare('insert into doctypes ('
          . join(",", @dt_fieldnames)
          . ') values ('
          . join(",", map { "?" } @dt_fieldnames)
          . ')');

    my $updater = $dbh->prepare('update doctypes set ' . join(",", map { "$_ = ?" } @dt_fieldnames) . ' where id = ?');

    my $dt_deleter = $dbh->prepare('delete from doctypes where id = ?');
    my @fieldspecs;
    ...
    while (my $line = <FH>) {
        ...
        if ($line =~ m!^doctype:\s+(\S+)\s*(.*)!i) {
            ...
            my %data =
              map { $_ => ($doctype{$_} || $dt_defaults{$_}) } @dt_fieldnames;

            my @args = map { $data{$_} } @dt_fieldnames;
    ...
    my %warned;
    my @unseen =
      grep { !$_->{_seen} && !$warned{$_->{id}}++ } values %doctypemap;
    foreach my $doctype (@unseen) {
        warn "Doctype '" . $doctype->{name} . "' from database is not " . "in $filename\n";
        delete $doctypemap{$doctype->{id}};
        delete $doctypemap{$doctype->{name}};
    }

    my $fieldspecmap = add_fieldspecs($dbh, \@fieldspecs, $fieldtypemap, \%doctypemap);
...
sub add_fieldspecs {
    ...
    my $fspec_inserter =
      $dbh->prepare('insert into fieldspecs ('
          . join(", ", @fs_fieldnames)
          . ') values ('
    ...
    my $fspec_updater =
      $dbh->prepare(
        'update fieldspecs set ' . join(", ", map { "$_ = ?" } @fs_fieldnames) . ' where doctypeid = ? and name = ?');

    my $fspec_deleter = $dbh->prepare('delete from fieldspecs where doctypeid = ? and name = ?');
    ...
    foreach my $fieldspec (@$fspeclist) {
        ...
        if (my $existing = $fieldspec_type_map{$fieldspec->{name}}) {
            if ($fieldspec->{type} != $existing->{type}) {
                ...
                die sprintf("Type '%s' for fieldspec '%s' does not match " . "existing type '%s' registered for '%s'.",
                    $typename, $fspecname, $etypename, $edoctype->{name} . ":" . $existing->{name});
            }
        } else {
        ...
        my %data = map { $_ => (defined $fieldspec->{$_} ? $fieldspec->{$_} : $defaults{$_}) } @fs_fieldnames;
        ...
        if (my $existing = $fieldspecmap{$k}) {
            if (my $c = hash_diff($existing, \%data, \@fs_fieldnames)) {
                ...
                $fspec_updater->execute(@args, $existing->{doctypeid}, $existing->{name});
            }
        } else {
    ...
    foreach my $key (keys %fieldspecmap) {
        ...
        unless ($fspec->{_seen}) {
            ...
            print "Deleting fieldspec " . $doctypename . ":" . $fspec->{name} . "\n";
            $fspec_deleter->execute($fspec->{doctypeid}, $fspec->{name});
...
sub add_editpages {
    ...
    my $inserter =
      $dbh->prepare(
        'insert into editpages (' . join(",", @fields) . ') values (' . join(",", map { "?" } @fields) . ')');

    my $updater =
      $dbh->prepare(
        'update editpages set ' . join(",", map { "$_ = ?" } @fields) . ' where doctypeid = ? and page = ?');

    my $deleter = $dbh->prepare('delete from editpages where doctypeid = ? and page = ?');
    ...
    while (<FH>) {
        ...
        if (/^DocType: (\w+)/) {
            ...
            die "Doctype mismatch $doctype->{name} <=> $doctypename"
              if ($doctype->{name} ne $doctypename);
            ...
            if (my $existing = $editpagemap{$key}) {
                ...
                my $changed = hash_diff($existing, \%data, \@fields);
                if ($changed) {
                    print "Updating editpage $doctypename:$editpage{page}: " . "$changed\n";
                    $updater->execute(@args, $existing->{doctypeid}, $existing->{page});
                }
    ...
    foreach my $key (keys %editpagemap) {
        ...
        unless ($page->{_seen}) {
            ...
            print "Deleting editpage " . $doctypename . ":" . $page->{page} . "\n";
            $deleter->execute($page->{doctypeid}, $page->{page});
...
sub hash_diff {
    ...
    if (@changed) {
        my %data = map { $_ => {(defined($h1->{$_}) ? $h1->{$_} : '<undef>') => $h2->{$_}} } @changed;
        $data{fieldlist} = "<changed>" if ($data{fieldlist});
        return Data::Dumper->new([ \%data ], ['changed'])->Terse(1)->Indent(0)->Useqq(1)->Dump;
    }
...
Lars Eskildsen <lars@magenta-aps.kd>
