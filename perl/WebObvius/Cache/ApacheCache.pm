...
sub with_db {
    ...
    eval {
        ...
        my %h;
        tie(%h, 'SDBM_File', $this->{cache_db}, O_RDWR|O_CREAT, 0666)
          or die "Failed to tie hash db";
        eval {$res = $method->(\%h);};
        untie %h;
        if ($@) {
            die $@;
...
sub perform_command_sophisticated_rightbox_clear {
    ...
    while (my @cur_docids = splice @docids, 0, 5000) {
        push @docids_to_clear,@{$this->check_rightboxes(\@cur_docids,['rightboxes', 'boxes1', 'boxes2', 'boxes3'])};
    }
...
sub clear_moved {
    ...
    for my $uri (@uris) {
        $uri .= '%';
        my $res = $this->execute_query(
            "select docid from docid_path dp natural join versions v
    ...
    for my $docid (@docids) {
        push @to_clear,
          @{
            $this->check_vfields_for_docids([{docid => $docid}],
                ['content', 'teaser','html_content', 'introduction','introduktion'])
          };
    }
...
sub find_dirty {
    ...
    my $vals =
      $cache_objects->request_values('uri', 'doctype', 'docid', 'clear_leftmenu','clear_recursively', 'document_moved');
    ...
    my @commands = grep { $_ }
      (@clear_recursively,@$referrers,@related,@$special_actions,@uris_to_clear,@$moved_documents,@clear_tags_command);
